// Fichier : server.js (VERSION FINALE VALIDÉE)
...
// 3. ROUTES PUBLIQUES (Pas de Token nécessaire)
  ...
  try {
    ...
    try {
        await resend.emails.send({
          ...
          subject: `🔥 Lead ImmoFlow: ${property.address}`,
          html: `<p>Nouveau client : ${firstName} ${lastName} (${phone})<br>Message: ${message}</p>`
        });
    } catch (e) {}
...
// 4. ROUTES PROTÉGÉES (Token requis)
...
app.put('/api/contacts/:id', authenticateToken, async (req, res) => {
    try {
        ...
        // Filtrage strict des données pour éviter les crashs
...
app.delete('/api/contacts/:id', authenticateToken, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        // Nettoyage des factures avant suppression (Méthode Nettoyeur)
        await prisma.invoice.deleteMany({ where: { contactId: id } });
...
app.get('/api/stats', authenticateToken, async (req, res) => {
    try {
        const [p, c, b, s, t] = await Promise.all([
            prisma.property.count(), prisma.contact.count(),
            prisma.contact.count({ where: { type: 'BUYER' } }), prisma.contact.count({ where: { type: 'SELLER' } }),
